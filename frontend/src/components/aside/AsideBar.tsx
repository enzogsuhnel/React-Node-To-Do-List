...
export default function AsideBar() {
  ...
  const options = [
    {
      ...
      icon: 'edit_square',
      subItems: taskLists,
      action: { text: 'Nova lista', icon: 'add', onclick: () => { navigate("task-list") } }
    },
    {
  ...
  const { getTaskLists , request} = apiContext;
  ...
  return (
    <aside
      ref={asidebarRef}
      className={`flex h-screen z-10 fixed top-[70px] ${menuOpen ? "visible w-[310px] items-start p-4" : "invisible md:visible w-0 md:w-[75px] items-center"} overflow-hidden shadow flex flex-col gap-4 justify-start  bg-primary py-4 transition-all duration-300 ease-in-out`}>
      {options.map((el, index) => {
        ...
        return (
          <div className="flex flex-col text-white" key={index}>
            ...
            >
              <div className={`p-2  rounded flex items-center justify-center ${isActive && !menuOpen && "bg-teal-600"}`}>
                <span
                  className="material-symbols-outlined">{el.icon}
                </span>
              </div>
              {menuOpen && (
                <label
            ...
            {
              menuOpen && el.subItems?.map((subItem, index) => (
                <div key={index} className="cursor-pointer px-4 py-1 my-1"
                  onClick={() => {
